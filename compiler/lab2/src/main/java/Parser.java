 ...
 */
public class Parser {
    static Set<Enclosure> enclosures = new HashSet<>();
    ...
    public static Set<Enclosure> rule2Enclosure(List<Pair<String, String>> rules, String beginSymbol) {
        ...
        //加入全部的内容
        enclosures.add(new Enclosure(rules));
            ...
            for (Enclosure e : enclosures) {
                ...
                if (!visited.contains(e)) {
                    ...
                    for (Enclosure next : nextEnclosure) {
                        if (!enclosures.contains(next))
                            newAddList.add(next);
                    }
                    visited.add(I);
                }
            }
            enclosures.addAll(newAddList);
            if (0 == newAddList.size()) break;
        }
